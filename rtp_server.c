...
int
rtp_server_init(struct rtp_server *rtpsrv, int mpx_samplerate, int max_samples, int baseport)
{
	return 0;
}
...
rtp_server_update_stats(gpointer user_data)
{
	...
	g_object_get(rtpsrv->rtpsink, "bytes-served", &ctl->rtp_bytes_sent, NULL);
	g_object_get(rtpsrv->rtcpsink, "bytes-served", &ctl->rtcp_bytes_sent, NULL);

	/* Do this only for rtpsink, they are supposed to have the same receivers
	 * anyway */
	g_object_get(rtpsrv->rtpsink, "clients", &clients, NULL);
	str_ptr = clients;
	while((token = strtok_r(str_ptr, ",", &str_ptr)) && (i < RTP_SRV_MAX_RECEIVERS)) {
...
rtp_server_add_receiver(int addr)
{
	...
	if(strstr(clients, ipv4string) == NULL)
		g_signal_emit_by_name(rtpsrv->rtpsink, "add", ipv4string, rtpsrv->baseport, NULL);
	g_free(clients);
	...
	if(strstr(clients, ipv4string) == NULL)
		g_signal_emit_by_name(rtpsrv->rtcpsink, "add", ipv4string, rtpsrv->baseport + 1, NULL);
	g_free(clients);
...
rtp_server_remove_receiver(int addr)
{
	...
	g_signal_emit_by_name(rtpsrv->rtpsink, "remove", ipv4string, rtpsrv->baseport, NULL);
	g_object_get(rtpsrv->rtpsink, "clients", &clients, NULL);
	if(strstr(clients, ipv4string) == NULL)
		rtpsinkok = 1;
	g_free(clients);
	...
	/* Same for rtcpsink */
	g_signal_emit_by_name(rtpsrv->rtcpsink, "remove", ipv4string, rtpsrv->baseport + 1, NULL);
	g_object_get(rtpsrv->rtcpsink, "clients", &clients, NULL);
	if(strstr(clients, ipv4string) == NULL)
		rtcpsinkok = 1;
	g_free(clients);
...
_rtp_server_init(void *data)
{
	...
	g_signal_connect(G_OBJECT(rtpsrv->msgbus), "message::error", (GCallback) rtp_server_error_cb, rtpsrv);
	...
	audio_caps = gst_caps_new_simple("audio/x-raw",
				"rate", G_TYPE_INT, rtpsrv->mpx_samplerate,
	...
	gst_bin_add_many(GST_BIN(rtpsrv->pipeline), rtpsrv->rtpsink, rtpsrv->rtcpsink, rtcpsrc, NULL);
